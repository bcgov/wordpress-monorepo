    ...
    public function upload_document(array $file, array $metadata = []) {
        ...
        if ($file['size'] > $this->config->get('max_file_size')) {
            return new WP_Error(
                ...
                ['max_size' => $this->config->get('max_file_size'), 'file_size' => $file['size']]
            );
        }
        ...
        if (empty($file_type['type']) || !$this->config->is_allowed_mime_type($file_type['type'])) {
            return new WP_Error(
                ...
                ['file_type' => $file_type, 'allowed_types' => $this->config->get('allowed_mime_types')]
            );
        }
        ...
        add_filter('wp_media_upload_handler', function($handler) {
