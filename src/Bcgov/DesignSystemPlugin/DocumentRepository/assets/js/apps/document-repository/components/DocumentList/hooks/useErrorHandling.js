...
const useErrorHandling = ({ onShowNotification }) => {
     ...
     */
    const retryOperation = useCallback(async (operation, handlers) => {
        const maxRetries = 3;
     ...
     * @param {Object} handlers - Handlers for different operation types
     */
    const retryAllOperations = useCallback((handlers) => {
        failedOperations.forEach(operation => retryOperation(operation, handlers));
